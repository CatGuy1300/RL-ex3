...
transitions_count = np.zeros((NUM_ACTIONS, NUM_STATES, NUM_STATES))
observed_rewards = np.zeros(NUM_STATES)
visited_states_count = np.zeros(NUM_STATES)

value_function = np.random.uniform(low=0, high=0.1, size=NUM_STATES)
transition_probabilities = np.ones((NUM_ACTIONS, NUM_STATES, NUM_STATES)) / NUM_STATES
...
while consecutive_no_learning_trials < NO_LEARNING_THRESHOLD:
    ...
    actions_rewards = np.zeros(NUM_ACTIONS)
    for action in range(NUM_ACTIONS):
        actions_rewards[action] += rewards[state]
        for next_state in range(NUM_STATES):
            actions_rewards[action] += (GAMMA * transition_probabilities[action][state][next_state] *
                                        value_function[next_state])
    action = int(actions_rewards.argmax())
    ...
    transitions_count[action][state][new_state] += 1
    observed_rewards[state] += R
    visited_states_count[new_state] += 1
    ...
    if new_state == NUM_STATES - 1:
        ...
        for action in range(NUM_ACTIONS):
            for current_state in range(NUM_STATES):
                if transitions_count[action][current_state].sum() != 0:
                    for next_state in range(NUM_STATES):
                        probability = (transitions_count[action][current_state][next_state] /
                                       transitions_count[action][current_state].sum())
                        transition_probabilities[action][current_state][next_state] = probability

        for current_state in range(NUM_STATES):
            if visited_states_count[current_state] != 0:
                rewards[current_state] = observed_rewards[current_state] / visited_states_count[current_state]
        ...
        while not stop:
            ...
            for current_state in range(NUM_STATES):
                actions_rewards = np.zeros(NUM_ACTIONS)
                for action in range(NUM_ACTIONS):
                    actions_rewards[action] += rewards[current_state]
                    for next_state in range(NUM_STATES):
                        actions_rewards[action] += (GAMMA * transition_probabilities[action][current_state][next_state]
                                                    * previous_value_function[next_state])
                new_value_function[current_state] = actions_rewards.max()
            if np.abs(new_value_function - previous_value_function).max() <= TOLERANCE:
                stop = True
            else:
                previous_value_function = new_value_function
                new_value_function = np.zeros_like(value_function)
